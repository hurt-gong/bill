...
public class IDCardUtil {
	...
	public static ResponseDTO IDCardValidate( String IDStr ) throws ParseException {
		ResponseDTO response = new ResponseDTO();
		response.setCode(ResponseHeadCodeEnum.SUCCESS.key());
		response.setMassage("身份证号校验成功.");
		String Ai = "";
		// 判断号码的长度 15位或18位  
		if ( IDStr.length() != 15 && IDStr.length() != 18 ) {
			response.setCode(ResponseHeadCodeEnum.ERROR.key());
			response.setMassage("身份证号码长度应该为15位或18位。");
			return response;
		}
		...
		if ( isNumeric(Ai) == false ) {
			response.setCode(ResponseHeadCodeEnum.ERROR.key());
			response.setMassage("身份证15位号码都应为数字 ; 18位号码除最后一位外，都应为数字。");
			return response;
		}
		...
		try {
			if ( (gc.get(Calendar.YEAR) - Integer.parseInt(strYear)) > 150
				...
				response.setCode(ResponseHeadCodeEnum.ERROR.key());
				response.setMassage("身份证生日不在有效范围");
				return response;
			}
		...
		if ( Integer.parseInt(strMonth) > 12 || Integer.parseInt(strMonth) == 0 ) {
			response.setCode(ResponseHeadCodeEnum.ERROR.key());
			response.setMassage("身份证月份无效");
			return response;
		}
		if ( Integer.parseInt(strDay) > 31 || Integer.parseInt(strDay) == 0 ) {
			response.setCode(ResponseHeadCodeEnum.ERROR.key());
			response.setMassage("身份证日期无效");
			return response;
		}
		...
		if ( areacode.get(Ai.substring(0, 2)) == null ) {
			response.setCode(ResponseHeadCodeEnum.ERROR.key());
			response.setMassage("身份证地区编码错误");
			return response;
		}

		if ( isVarifyCode(Ai, IDStr) == false ) {
			response.setCode(ResponseHeadCodeEnum.ERROR.key());
			response.setMassage("身份证校验码无效，不是合法的身份证号码");
			return response;
		}
		return response;
	}
