...
require(['jqtransform','headerSlideDown','highstock','My97DatePicker'], function(jqtransform,headerSlideDown,highstock,My97DatePicker) {
	...
	var name='数量';
	var param = {};
	var url ='/manage/school/statistics/getLoginStaChart.html';
	param.selType =$('.adminTopBar a.btn').attr('value');
	param.startDate =$("#time").val();
	param.endDate = $("#time1").val();
	param.gradeId =$('select[name="gradeId"]').val();
	param.subjectId =$('select[name="subjectId"]').val();
	...
	$(".adminTopBar>a").on("click",function(){
		$('input[name="selType"]').val($(this).attr('value'));
		$('#quaForm').submit();
	 });
