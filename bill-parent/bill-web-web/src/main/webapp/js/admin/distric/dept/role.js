...
require(['jqtransform','headerSlideDown','dialog'], function(jqtransform,aaaa,dialog) {
	 ...
	 $('body').on('click','.cul li span',function(){
	 	window.location.href = '/manage/distric/role/list.html?id='+$(this).attr('id');
	 });

	 //删除角色名称

	 $('body').on('click','.cul li .del', function(){
	 	$('.clearBox .sure').attr('value',$(this).siblings('span').attr('id'));
		$('.delRole').show();
	 });

	 $('body').on('click', '.delRole .sure',function(){
	 	window.location.href= '/manage/distric/role/del.html?id='+$(this).attr('value');
	 });
	...
	//  });
	 ...
	 $('.addAlert .sure').on('click',function(){
		...
		if(!name){
	 	...
	 	if(name){
			$.ajax({
				url:'/manage/distric/role/add.html',
				type:"post",
				data:param,
				dataType:"json",
...
$(this).parent().hide();	
	 ...
	 $('.updateAlert .sure').on('click',function(){
	 	...
	 	if(name){
			$.ajax({
				url:'/manage/distric/role/update.html',
				type:"post",
				data:param,
				dataType:"json",
				success:function(msg){
