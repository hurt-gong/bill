...
require(['jqtransform','headerSlideDown'], function(jqtransform,aaaa) {
	 ...
	 $("#btn").click(function(){
			 ...
			 if($('.onerole li').find('.jqTransformChecked').size()==0){
			 $('.firstLi a').removeClass('jqTransformChecked');
			}
	 ...
	 $('.roleGroup span').click(function(){
	 	...
	 	if($(this).hasClass('animate')){
	 		$(this).parent().stop().animate({'height':50},500);
	...
	$('.roleGroup li').find('p').on('click',function(){
 	...
 	$(".sure").click(function(){
 		...
 		// $("#addPer").action="/manage/distric/permission/user/addPermissions.html";
 		//$("#addPer").submit();
 	});
 	function subform(){
	...
	$(".cancel").click(function(){
		...
		window.location.href="/manage/distric/permission/user/list.html?deptId="+deptId+"&deptPId="+deptPId
